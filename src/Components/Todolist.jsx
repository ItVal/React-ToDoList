  ...
  return (
    <div className="bg-blue-600 w-3/6 rounded-sm shadow-lg">
      ...
      <button
        type="button"
        onClick={() => {
          if (todoTitle.length !== 0 && todoTitle.trim().length !== 0)
            addTodo(todoTitle);
          setTodoTitle("");
        }}
        className="text-gray-100 m-3 h-5"
      >
        {<FaPlus size={20} />}
      </button>
          ...
          {isFormUpdate && index === numero && (
            <div>
              <input
                ...
                className="w-5/6 h-8 m-3 outline-none  text-gray-900"
              />
              <button
                type="button"
                onClick={() => upDateTask(index, valueUpdate)}
                className="text-gray-100 m-3 h-5"
              >
                {<FaEdit />}
              </button>
